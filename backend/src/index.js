...
function connectToESP32() {
  ...
  if (USE_FALLBACK) {
    console.log('Fallback mode enabled, using mock data without attempting ESP32 connection');
  ...
  try {
    ...
    httpRequest = http.get(ESP32_DATA_ENDPOINT, options, (response) => {
      ...
      if (response.statusCode !== 200) {
        console.error(`Error connecting to ESP32: HTTP status ${response.statusCode}`);
      ...
      heartbeatTimer = setInterval(() => {
        ...
        if (now - lastMessageTime > 15000) {
          console.warn('Connection appears stalled - no messages for 15 seconds');
          broadcastConnectionStatus(false, 'Connection to ESP32 stalled. Reconnecting...');
...
function handleConnectionError(error) {
  console.error('ESP32 SSE connection error:', error.message || 'Unknown error');
  broadcastConnectionStatus(false, `Connection to ESP32 lost: ${error.message || 'Unknown error'}. Reconnecting...`);
  ...
  if (USE_FALLBACK && reconnectAttempts >= 1) {
    console.log('Using fallback/mock data mode after failed connection attempt');
    startMockDataGeneration();
    return;
  }
  ...
  console.log(`Reconnecting to ESP32 in ${delay}ms (attempt ${reconnectAttempts})`);
...
app.post('/api/reconnect-esp32', (req, res) => {
  ...
  if (USE_FALLBACK) {
    console.log('Manual reconnection requested - attempting real connection despite fallback mode');
  }
  ...
  try {
    ...
    if (connected) {
      ...
      server.listen(PORT, () => {
        ...
        console.log(`Live data stream available at http://localhost:${PORT}/api/live-data`);
        console.log(`======================================`);
